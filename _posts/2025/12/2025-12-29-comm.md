...
![四卡 AllToAll：颜色表示来源 rank，块内标注来源与目标 rank，交换前后可追踪每个分块](/img/2025/12/29/alltoall-rank-exchange.png)

图中每块 $B_{s\to d}$ 都标着“来源 $s$ → 目标 $d$”，颜色始终跟随来源 rank。例如，橙色的 $1\to2$ 块原先位于 rank 1 的目标 2 槽位，交换后出现在 rank 2 的来源 1 槽位。每卡的发送缓冲区 $X_s$ 和接收缓冲区 $Y_d$ 都可按 $4\times c$ 理解，其中 $c$ 是每块的元素数；交换满足 $Y_d[s,:]=X_s[d,:]$，块内元素不变，也不做规约。$s=d$ 的块仍归本卡。这里画的是逻辑分块与归属变化，不表示物理存储或网络拓扑。

发送按目标 rank 分块、接收按来源 rank 排列的约定见 [NCCL AlltoAll 文档](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/api/colls.html#ncclalltoall)。
