...
4. 加偏置：如果 skip_bias_add=False，在当前 rank 加全量偏置并返回 (output, None)；否则返回 (output, bias) 让上游融合。
目的：通过按输入维切分权重/输入，减少每个 rank 的参数和计算；输出端用 reduce 聚合回完整输出，或在 MoE/sequence parallel 场景下保持/组合分片以继续并行。文件参考：
...
1. 切分方式：
    - linear_fc1 默认用 ColumnParallelLinear（通过 build_module 注入，见 MLP.__init__），权重按输出维切分，每个 rank 产出自己那份中间激活；gather_output=False，所以保持分片。
